...
We will now begin to add some pointers to the map. This will involve defining
data about what we want to show on the map and where; then building a new React
component that will display an icon at the positions we specified on the map.
...
This data will live inside of the Map component, since it will be responsible
for rendering the pointers to the map.

Take the following data and copy it to the `Map.jx` file, above the `Map`
constant.
...
Next we will add a `<Pointer />` component that will show each location on the
...
Next we will create the `Pointer` class in the `Pointer.jsx` file. For now let's
add a simple render method that will return a `<div />` that will control the
styles for our pointer. We will add a `style` prop that sets the `left` and
`top` properties of the pointer which we read form the `x` and `y` values of our
data.
...
Notice that we are using the ES6 destructuring assignment [^1] here to pull the
`x` and `y` props into constants. This is a nice shorthand instead of having to
access the values manually.
...
simply import `PropTypes` and set the `propTypes` value of our class.
...
unions of types. Look at the documentation for more complex examples.
...
Notice that we set `position: absolute` on the pointer. Together with the `top`
and `left` styles that are applied in the React component, this is how the
Pointer will be placed on the map.
...
In this new code we are mapping over the `points` data array and for every
point, we return a `<Pointer />` and we destructure the `point` object as the
props. This is the same as passing:

```jsx
<Pointer x={point.x} y={point.y} details={point.details} />;
```

Finally we pass a `key` prop. The `key` is important when you are mapping over
data, this allows React to differentiate between each item in the list so that
it can determine which ones have updated [^3]. The key value must be unique for
