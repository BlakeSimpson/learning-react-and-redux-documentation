...
Now that we have the pointers displayed on the map, the next step is to make
them clickable so that we can display the "details" information from our data in
a popup over each point.
...
We will track weather a popup should be shown or not be using state within the
...
When adding a constructor you must _always_ call `super` and pass the props to
...
case it will be a link click) does not bubble up to the browser.
...
triggers the life cycle [^2] to run again (e.g. calling `componentDidUpdate` and
`render` for you).

Now we have to bind the `toggle` method to `this` in the constructor so that the
...
Now we will apply the `onClick` handlers to the pointer itself and the close
link so that clicking either of these elements will call the `toggle` method.
Update the `render` method to add the `onClick` handlers.
...
Finally in the `render` method we will build a custom set of class names to pass
...
(Note: We have to use the ES6 `[]` object assignment syntax instead of just
`hidden:` because we are using CSS modules - the class name is not `.hidden` but
a generated hash).
...
It would in theory be possible to call the `.bind(this)` the method inside the
...
changed but since calling `bind` creates a new function, every time render is
called new functions are being created, making it impure. React can then never
know for sure what has changed and will always call the `render` method even if
the DOM would have been identical [^3].
